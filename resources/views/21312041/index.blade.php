...
<table class="table" id="example1">
  ...
  <tbody>
    @forelse ($21312041_mahasiswa as $key => $item)
    <tr>
      <td>{{$key + 1}}</td>
      <td>{{$item->npm}}</td>
      <td>{{$item->nama}}</td>
      <td>{{$item->alamat}}</td>
      <td>
        <form action="/21312041/{{ $item->id }}" method="POST" id="deleteForm">
            <a href="/21312041/{{ $item->id }}/edit" class="btn btn-warning btn-sm">Edit</a>
            @csrf
            @method('delete')
